...
class AttachmentView(ArticleMixin, FormView):
    ...
    def dispatch(self, request, article, *args, **kwargs):
        if article.can_moderate(request.user):
            self.attachments = models.Attachment.objects.filter(articles=article, current_revision__deleted=False).exclude(current_revision__file=None).order_by("original_filename")
    ...
    def form_valid(self, form):
        ...
        if isinstance(attachment_revision, list):
            messages.success(
                self.request,
                _("Successfully added: %s") % (", ".join([ar.get_filename() for ar in attachment_revision])),
            )
        else:
            messages.success(
                self.request,
...
class AttachmentReplaceView(ArticleMixin, FormView):
    ...
    def form_valid(self, form):
        try:
            ...
            messages.success(
                self.request,
                _("%s uploaded and replaces old attachment.") % attachment_revision.get_filename(),
            )
            self.article.clear_cache()
        ...
        if self.can_moderate:
            if form.cleaned_data["replace"]:
                ...
                try:
                    ...
                    msg = "{attachment} does not contain any revisions.".format(attachment=str(self.attachment.original_filename))
...
class AttachmentAddView(ArticleMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        if not self.attachment.articles.filter(id=self.article.id):
            ...
            messages.error(
                self.request,
                _('"%(att)s" is already referenced.') % {"att": self.attachment.original_filename},
            )
        return redirect("wiki:attachments_index", path=self.urlpath.path, article_id=self.article.id)
...
class AttachmentDeleteView(ArticleMixin, FormView):
    ...
    def form_valid(self, form):
        if self.attachment.article == self.article:
            ...
            messages.info(
                self.request,
                _("This article is no longer related to the file %s.") % self.attachment.original_filename,
            )
        self.article.clear_cache()
...
class AttachmentSearchView(ArticleMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if not self.query:
            ...
            qs = qs.filter(Q(original_filename__contains=self.query) | Q(current_revision__description__contains=self.query) | Q(article__current_revision__title__contains=self.query))
